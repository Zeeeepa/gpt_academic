    ...
    def consolidate(self, output_path: str) -> bool:
        ...
        try:
            ...
            if not self.dry_run:
                ...
                # Auto-format the consolidated file
                self.log("\nFormatting consolidated file...")
                if format_standalone_file(output_path):
                    self.log("✓ Auto-formatting applied")
                else:
                    self.log("⚠ Auto-formatting encountered issues", 'warning')
            else:
                elapsed_time = time.time() - self.start_time
                self.log(f"\n{'='*70}")
...
def format_standalone_file(filepath: str) -> bool:
    """
    Apply formatting to a standalone consolidated file.
    
    Args:
        filepath: Path to the standalone Python file
        
    Returns:
        True if formatting succeeded, False otherwise
    """
    logger.info(f"Formatting standalone file: {filepath}")
    
    try:
        # Run formatters in sequence
        formatters = [
            ["ruff", "check", "--fix", filepath, "--quiet"],
            ["ruff", "format", filepath],
            ["isort", "--profile", "black", filepath, "--quiet"],
            ["black", filepath, "--quiet"],
        ]
        ...
        for formatter_cmd in formatters:
            try:
                ...
                logger.warning(f"Formatter {formatter_cmd[0]} reported issues (non-critical)")
                # Continue anyway - formatting issues shouldn't block
        
        logger.info(f"✓ Successfully formatted: {filepath}")
        return True
        
    except Exception as e:
        logger.error(f"Failed to format standalone file: {e}")
        return False
